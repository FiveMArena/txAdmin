...
RegisterNetEvent('txsv:req:tpToPlayer', function(tgtId)
  ...
  if allow then
    if GetConvarBool("arena:txAdmin:interceptTp") then
      TriggerEvent("arena:txAdmin:tpToPlayer", src, tgtId)
    else
...
RegisterNetEvent('txsv:req:bringPlayer', function(id)
  ...
  local allow = PlayerHasTxPermission(src, 'players.teleport')
  if allow then
    if GetConvarBool("arena:txAdmin:interceptBring") then
      TriggerEvent("arena:txAdmin:bringPlayer", src, id)
    else
